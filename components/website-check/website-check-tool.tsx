...
import { ReportLeadForm, WebsiteCheckSuccess, type WebsiteCheckPreview } from "@/components/website-check/report-lead-form";
...
function WebsiteCheckResults({ result }: { result: WebsiteCheckResult }) {
    ...
    <div className="mt-14">
      <section className="rounded-[2.5rem] border border-border bg-background/80 p-6 shadow-premium backdrop-blur-xl sm:p-10 lg:p-14" aria-labelledby="website-check-result-title">
        <div className="flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.2em] text-muted-foreground">{t.report}</p>
            <h2 ref={titleRef} tabIndex={-1} id="website-check-result-title" className="mt-4 max-w-4xl text-4xl font-semibold tracking-[-0.055em] outline-none sm:text-6xl">
              {t.resultFor} {result.domain}
            </h2>
          ...
          <div className="rounded-2xl border border-border bg-muted/35 px-5 py-4 text-sm text-muted-foreground">
            <p><span className="font-medium text-foreground">{t.test}:</span> {result.strategy === "mobile" ? "Mobile" : "Desktop"}</p>
            <p className="mt-1"><span className="font-medium text-foreground">{t.analysis}:</span> {new Date(result.createdAt).toLocaleString(locale === "de" ? "de-DE" : locale === "sq" ? "sq-AL" : "en-GB")}</p>
          </div>
        </div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {Object.values(result.scores).map((score) => <ScoreCard key={score.key} score={score} />)}
        </div>
...
export function WebsiteCheckTool() {
  ...
  const [preview, setPreview] = useState<WebsiteCheckPreview | null>(null);
  const [result, setResult] = useState<WebsiteCheckResult | null>(null);
  const [analysisToken, setAnalysisToken] = useState("");
  const [marketingConfirmationSent, setMarketingConfirmationSent] = useState(false);
  ...
  async function analyze(event: FormEvent<HTMLFormElement>) {
    ...
    setResult(null);
    setAnalysisToken("");
    setMarketingConfirmationSent(false);
    ...
    try {
      const response = await fetch("/api/website-check/analyze", {
        ...
        body: JSON.stringify({ url: trimmed, strategy, locale }),
      });
      ...
      trackWebsiteCheckEvent({
        event: "website_check_completed",
        domain: payload.preview.domain,
        strategy: payload.preview.strategy,
      });
    } catch (analysisError) {
      setError(analysisError instanceof Error ? analysisError.message : t.genericError);
      setState("error");
    }
  }
  ...
  return (
    <div>
      <form onSubmit={analyze} noValidate className="mx-auto max-w-5xl rounded-[2.25rem] border border-border bg-card/85 p-5 shadow-premium backdrop-blur-xl sm:p-8" aria-describedby={`${inputId}-help ${inputId}-error`}>
        ...
        <div className="mt-2 grid gap-3 lg:grid-cols-[1fr_auto]">
          <input
            ...
            className="min-h-16 w-full rounded-2xl border border-border bg-background/90 px-5 text-lg text-foreground outline-none transition placeholder:text-muted-foreground focus:border-foreground focus:ring-2 focus:ring-accent/35 disabled:opacity-60"
          />
          <button type="submit" disabled={state === "analyzing"} className="inline-flex min-h-16 items-center justify-center rounded-2xl bg-foreground px-7 text-base font-medium text-background shadow-premium transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:pointer-events-none disabled:opacity-60">
            {state === "analyzing" ? t.running : t.submit}
          ...
          <fieldset className="flex items-center gap-2" disabled={state === "analyzing"}>
            ...
            {(["mobile", "desktop"] as const).map((option) => (
              <label key={option} className={`cursor-pointer rounded-full border px-4 py-2 text-sm transition ${strategy === option ? "border-foreground bg-foreground text-background" : "border-border bg-background/70 text-muted-foreground hover:text-foreground"}`}>
                <input type="radio" name="strategy" value={option} checked={strategy === option} onChange={() => setStrategy(option)} className="sr-only" />
        ...
        <div className="mt-10 min-h-[22rem]">
          {state === "idle" && (
            <div className="grid min-h-[22rem] place-items-center rounded-[2rem] border border-dashed border-border bg-background/35 p-8 text-center">
              <div className="max-w-2xl">
                ...
                <h2 className="mt-5 text-2xl font-semibold tracking-[-0.04em]">{t.idleTitle}</h2>
                <p className="mt-3 leading-7 text-muted-foreground">{t.idleText}</p>
              </div>
            </div>
          )}
          ...
          {state === "error" && (
            <div className="grid min-h-[22rem] place-items-center rounded-[2rem] border border-red-500/30 bg-red-500/7 p-8 text-center">
              <div className="max-w-2xl">
                <h2 className="text-2xl font-semibold tracking-[-0.04em]">{t.errorTitle}</h2>
                <p className="mt-3 leading-7 text-muted-foreground">{error}</p>
                <p className="mt-3 text-sm text-muted-foreground">{t.errorHint}</p>
              </div>
            </div>
          )}
        </div>
      ...
      {state === "success" && preview && analysisToken && !result && (
        <ReportLeadForm
          preview={preview}
          analysisToken={analysisToken}
          onSuccess={(unlockedResult, confirmationSent) => {
            setResult(unlockedResult);
            setMarketingConfirmationSent(confirmationSent);
          }}
        />
      )}
      ...
      {state === "success" && result && (
        <>
          <WebsiteCheckResults result={result} />
          <WebsiteCheckSuccess marketingConfirmationSent={marketingConfirmationSent} />
        </>
      )}
    </div>
  );
