...
export default async function WebsiteCheckReportPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const pdfUrl = `/website-check/report/${encodeURIComponent(token)}/pdf`;
      ...
      <article className="mx-auto mt-12 max-w-7xl">
        <div className="rounded-[2.5rem] border border-border bg-card/90 p-7 shadow-premium backdrop-blur-xl sm:p-12 lg:p-16">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            ...
            <a href={pdfUrl} download className="inline-flex min-h-14 shrink-0 items-center justify-center rounded-full border border-border bg-background px-7 font-medium shadow-sm transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">Report als PDF herunterladen <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" /></a>
          </div>
        ...
        {result.intelligence ? (
          <SiteIntelligenceResults intelligence={result.intelligence} />
        ) : (
          <section className="mt-8 rounded-[2.5rem] border border-border bg-background/80 p-8 text-center shadow-premium sm:p-12">
            <h2 className="text-3xl font-semibold tracking-[-0.04em]">Erweiterte Analyse nicht gespeichert</h2>
            <p className="mx-auto mt-4 max-w-2xl leading-7 text-muted-foreground">Dieser Report stammt aus einer älteren Analyse. Starten Sie einen neuen Check, damit auch die Bereiche 1 bis 19 im Report und im PDF enthalten sind.</p>
            <Link href="/website-check" className="mt-7 inline-flex min-h-14 items-center justify-center rounded-full bg-foreground px-8 font-medium text-background">Neuen Check starten <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" /></Link>
          </section>
        )}

        <section className="mt-8 rounded-[2.5rem] border border-border bg-card/90 p-7 shadow-premium backdrop-blur-xl sm:p-12">
          <div className="rounded-[2rem] border border-border bg-muted/35 p-7 sm:p-10"><h2 className="text-3xl font-semibold tracking-[-0.04em]">Was sind die nächsten sinnvollen Schritte?</h2><p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">Die Ergebnisse sind eine technische Momentaufnahme und ersetzen keine vollständige fachliche Prüfung. In einem unverbindlichen Gespräch können die Befunde nach geschäftlicher Relevanz, Aufwand und realistischer Wirkung priorisiert werden.</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Link href="/#kontakt" className="inline-flex min-h-14 items-center justify-center rounded-full bg-foreground px-8 font-medium text-background shadow-premium transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">Unverbindliches Erstgespräch vereinbaren <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" /></Link><a href={pdfUrl} download className="inline-flex min-h-14 items-center justify-center rounded-full border border-border bg-background px-8 font-medium transition hover:bg-muted">PDF herunterladen</a></div></div>
        </section>

        <p className="mx-auto mt-8 max-w-3xl text-center text-sm leading-6 text-muted-foreground">Der Report basiert auf der Google PageSpeed Insights API und einer automatisierten Analyse öffentlich sichtbarer Signale. Messwerte können schwanken. Heuristische Erkennungen sind keine Garantie und ein hoher Score garantiert weder Rankings noch Umsatz.</p>
      </article>
    </main>
  );
}
